          ...
          <v-col class="d-flex flex-column text-center justify-end">
            ...
            <v-sheet class="mx-auto" style="width: 100%;">
              <v-slide-group v-model="model" class="pa-4" show-arrows>
                <v-slide-item
                  v-for="(item, index) in json.carouselData"
                  :key="index"
              ...
              <v-expand-transition>
                <v-sheet v-if="model != null" color="grey lighten-4" height="200" tile>
                  <v-row class="fill-height" align="start" style="text-align: left">
                    <v-col cols="12" md="6">
                      <h3 class="title">{{ json.carouselData[model].name }}</h3>
                    </v-col>
                    <v-col cols="12" md="6">
...
export default {
  ...
  methods: {
    addToOrder(item, price) {
      ...
      this.orderTotal += parseInt(price);
    },
    serverCall() {
      console.log(`Send to server: ${this.order}`);
      this.order = [];
    }
  }
};
